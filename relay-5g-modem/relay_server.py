    ...
    def listen_for_clients(self) -> None:
        ...
        while True:
            try:
                ...
                RelayClient.client_data, RelayClient.client_address = (
                    self.RELAY_SOCKET.recvfrom(1024)
                )
                # handle client requests in a new thread
                RelayClient.startClientThread(
                    RelayClient.handle_client,
                    RelayClient.client_data,
                    RelayClient.client_address,
                )
